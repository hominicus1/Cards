(function(root,factory){
  ...
  function handAnalysis(hand,valueCards=hand,context={}){
    ...
    const positionBonus=context.position==='forehand'?2:context.position==='rear'?1:0,grade=score=>score>=108?'bardzo mocna':score>=90?'mocna':score>=70?'grywalna':score>=48?'ryzykowna':'słaba';
    const sideShape=trumpSuit=>{const groups=['D','H','S','C'].filter(s=>s!==trumpSuit).map(s=>hand.filter(c=>!isJack(c)&&c.suit===s)),voids=groups.filter(x=>!x.length).length,singletons=groups.filter(x=>x.length===1).length,protectedTens=groups.filter(x=>x.some(c=>c.rank==='A')&&x.some(c=>c.rank==='10')).length,nakedTens=groups.filter(x=>x.some(c=>c.rank==='10')&&!x.some(c=>c.rank==='A')).length;return {voids,singletons,protectedTens,nakedTens};};
    for(const suit of ['D','H','S','C']){
      const t=tops(valueCards,'suit',suit),value=(t.count+1)*SUIT_BASE[suit],trumps=hand.filter(c=>isTrump(c,{type:'suit',suit})).length,shape=sideShape(suit),parts=[['triomfy',trumps*9],['Asy',aces*12],['dziesiątki',tens*7],['oczka',Math.round(high*.25)],['chronione 10',shape.protectedTens*4],['braki kolorów',shape.voids*4],['krótkie kolory',context.beforeSkat?shape.singletons*2:0],['samotne 10',shape.nakedTens*-5],['najwyższy dupek',t.with?4:0],['pozycja',positionBonus]],score=parts.reduce((n,x)=>n+x[1],0);
      const risk=trumps<5||shape.nakedTens>=2||!t.with&&t.count<2?'wysokie':trumps>=7&&aces>=2&&!shape.nakedTens?'niskie':'średnie',handReady=score>=92&&trumps>=6&&(t.with||jacks>=2)&&aces>=1&&shape.nakedTens<=1;
      const handReasons=[`${trumps} triomfów`,`${aces} ${aces===1?'As':'Asy'}`];if(shape.voids)handReasons.push(`${shape.voids} ${shape.voids===1?'brak koloru':'braki kolorów'}`);if(!shape.nakedTens)handReasons.push('bez samotnej 10');
      options.push({type:'suit',suit,value,handValue:(t.count+2)*SUIT_BASE[suit],score,grade:grade(score),breakdown:parts.filter(x=>x[1]),trumps,...shape,handReady,handReasons,label:`${SUIT_NAMES[suit]} · ${t.with?'z':'bez'} ${t.count}, gra ${t.count+1} × ${SUIT_BASE[suit]} = ${value}`,risk});
    }
    const gt=tops(valueCards,'grand'),grandValue=(gt.count+1)*24,gshape=sideShape(null),gparts=[['dupki',jacks*15],['Asy',aces*14],['dziesiątki',tens*8],['oczka',Math.round(high*.2)],['chronione 10',gshape.protectedTens*4],['braki kolorów',gshape.voids*3],['samotne 10',gshape.nakedTens*-6],['pozycja',positionBonus]],grandScore=gparts.reduce((n,x)=>n+x[1],0),grandReady=grandScore>=94&&jacks>=3&&aces>=2&&gshape.nakedTens<=1;
    options.push({type:'grand',value:grandValue,handValue:(gt.count+2)*24,score:grandScore,grade:grade(grandScore),breakdown:gparts.filter(x=>x[1]),jacks,...gshape,handReady:grandReady,handReasons:[`${jacks} dupki`,`${aces} Asy`,gshape.nakedTens?'kontrolowane ryzyko dziesiątki':'bez samotnej 10'],label:`Grand · ${gt.with?'z':'bez'} ${gt.count}, gra ${gt.count+1} × 24 = ${grandValue}`,risk:jacks<2||gshape.nakedTens>=2?'wysokie':jacks>=3&&aces>=2&&!gshape.nakedTens?'niskie':'średnie'});
    const lowCards=hand.filter(c=>['7','8','9'].includes(c.rank)).length,nullRawDanger=hand.reduce((n,c)=>n+({A:10,K:7,Q:5,J:3,'10':2}[c.rank]||0),0),nullDanger=Math.max(0,nullRawDanger-lowCards*2),nullScore=Math.max(0,60-nullDanger),nullReady=nullScore>=48&&nullDanger<=12;
    options.push({type:'null',value:23,handValue:35,score:nullScore,grade:grade(nullScore),breakdown:[['baza',60],['wysokie karty',nullRawDanger*-1],['niskie wyjścia',lowCards*2]],handReady:nullReady,handReasons:[`${lowCards} niskie karty`,`zagrożenie ${nullDanger}`],label:'Null · stała wartość 23',risk:nullDanger>24?'wysokie':nullDanger<=12?'niskie':'średnie'});
    options.sort((a,b)=>b.score-a.score);return {options,recommended:options[0],cardPoints:high,aces,tens,jacks};
  }
  function aiBidLimit(hand,context={}){const a=handAnalysis(hand,hand,context),best=a.options[0];return best.score>=70?best.value:best.score>=48?Math.min(best.value,36):0;}
