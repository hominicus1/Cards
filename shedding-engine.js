(function(root,factory){
  ...
  function analyzePlay(rules,cards,topCard,{opening=false}={}){
    ...
    if(packets.length===1){
      ...
      if(size===3&&!packet.cards.some(c=>c.suit===cfg.tripleRequiresSuit))return {valid:false,reason:'Trójka musi zawierać kiera'};
    } else {
      ...
      if(packets.some(p=>p.cards.length===3&&!p.cards.some(c=>c.suit===cfg.tripleRequiresSuit)))return {valid:false,reason:'Każda trójka w drabince musi zawierać kiera'};
  ...
  function enumeratePlays(rules,hand,topCard,{opening=false}={}){
    ...
    for(const p of flat){const a=analyzePlay(rules,p,topCard,{opening});if(a.valid)plays.push({cards:p,analysis:a});}
    ...
    for(let mask=1;mask<(1<<n);mask++){
      ...
      const a=analyzePlay(rules,selected,topCard,{opening});if(a.valid)plays.push({cards:selected,analysis:a});
    }
    const seen=new Set();
  ...
  function play(state,rules,playerId,cardUids){
    ...
    const analysis=analyzePlay(rules,selected,state.pile.at(-1),{opening:state.opening});
  ...
  function settleAfterMove(state,rules,playerId){
    ...
    if(alive.length===1){
      ...
      if(rules.shedding.lastPlayerCanEscape){
        const escapes=enumeratePlays(rules,last.hand,state.pile.at(-1));
